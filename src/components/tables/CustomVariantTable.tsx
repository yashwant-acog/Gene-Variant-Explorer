...
export default function CustomVariantTable({
  ...
  // Unique groups for the double-header, only for visible columns
  const groups = Array.from(new Set(columns.map((c) => c.group)));

  return (
    <div className="overflow-x-auto border border-gray-200 dark:border-scientific-border rounded-lg shadow-sm">
      <table className="w-full text-left border-collapse">
        <thead className="sticky top-0 z-20">
            ...
            {groups.map((group) => {
              ...
              return (
                <th
                  key={group}
                  colSpan={span}
                  className="px-4 py-1 text-center border-r border-gray-200 dark:border-scientific-border last:border-r-0"
                >
                  {group}
                </th>
              );
          ...
          {/* Main Field Headers */}
          <tr className="bg-white dark:bg-scientific-panel/80 border-b border-gray-200 dark:border-scientific-border text-xs font-semibold text-gray-700 dark:text-gray-200">
            {columns.map((col, idx) => (
              <th
                key={idx}
                className="px-4 py-3 border-r border-gray-200 dark:border-scientific-border last:border-r-0 whitespace-nowrap"
              >
                {col.label}
              </th>
            ))}
